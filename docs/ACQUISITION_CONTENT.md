...
Reviewed September 22, 2026 for Desktop **1.4.5.7**.
...
Recipe references are the corresponding item pages on https://terraria.wiki.gg/wiki/. Checked against modern Desktop references, including the updated Amazon, Blade of Grass, Hive-Five, Spider equipment, Flinx Staff and gem-staff recipes. Diamond and Ruby Staff cost 7 metal bars in 1.4.5.7, not the older 10. Hive-Five uses only 14 Bee Wax. The source links remain available in each preview. Full-set totals use the corresponding armor pages.
...
Run `node scripts/verify-acquisition.mjs` to check content coverage, duplicate catalog shadowing, official source URLs, positive output/ingredient quantities, unique alternatives, valid conditional/ranged percentages, and PNG assets. The verifier includes regression checks for current gem-staff and Hive-Five recipes.
