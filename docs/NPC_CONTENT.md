...
September 22, 2026 audit: reviewed all 28 records and progression/condition groups, cross-checking the version-sensitive shop and recruitment rules against the sources above and the installed game's 1.4.5.7 changelog. Restored the Traveling Merchant's missing [Black and Yellow Counterweights](https://terraria.wiki.gg/wiki/Counterweights#From_Traveling_Merchant); all six counterweights now have vendor listings. Kept Format:C out of the Skeleton Merchant's stock because 1.4.5.7 changed it to crafting. Modern pylon sales no longer require happiness, and the Tavernkeep exclusion is intentional. Purely cosmetic additions such as Giant Tiki and Pal Metal vanity remain outside scope.

`findNpcs` centralizes search across names, recruitment, services, shop conditions and displayed unlock-stage labels. It accepts multiple words and normalizes punctuation, so `post plantera` finds the Truffle's Autohammer group and `leaf wings jungle` finds the Witch Doctor. Regression checks cover these queries, era filtering, missing yoyo supplies, invalid stages and accidentally duplicated shop rows.
