  ...
  return <section className="panel plaque checklist" aria-label="Boss and event checklist">
    ...
    {groups.map(group => {
      ...
      return <section className="checklist-era" key={group.name}><h3>{group.name}<small>{group.note}</small></h3>{visible.length ? visible.map(row => <div className={`checklist-row ${checked.includes(row.id) ? 'is-defeated' : ''}`} key={row.id}>
        ...
        <details><summary><span className={`checklist-portrait${!checklistArt[row.id] && row.art.length > 1 ? ' has-multiple' : ''}`}>{checklistArt[row.id] ? <img src={`/items/${checklistArt[row.id]}`} alt="" /> : row.art.map(id => <img key={id} src={`/bosses/${id}.png`} alt="" />)}</span><span className="checklist-name">{row.name}{(row.optional || row.event) && <small>{row.event ? 'Event' : 'Optional'}</small>}</span><span className="checklist-loot-label">Details ⌄</span></summary><div className="checklist-details"><ChecklistSpawn encounterId={row.id} />{renderLoot(row)}{row.stageId && <button className="checklist-gear" onClick={() => onGear(row.stageId)}>View gearing stage →</button>}</div></details>
