...
import { cropInventoryFrame } from './sprite-frames.mjs'
...
for (const [file, id] of Object.entries(manifest)) {
  ...
  const outputPath = fileURLToPath(new URL(file, outputDirectory))
  fs.writeFileSync(outputPath, new Uint8Array(await output.data.arrayBuffer()))
  cropInventoryFrame(outputPath)
}
